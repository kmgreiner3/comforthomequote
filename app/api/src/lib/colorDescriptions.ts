// Bundled copy of the color descriptions used to build Nova Canvas prompts.
// Source of truth for the copy itself is app/web/src/content/colorInfo.ts
// (Dylan Nadeau, docs/client/color-descriptions.md); this module duplicates
// only the `description` text (no imagery) so app/api can build prompts
// without depending on the web app. Names must match `SHINGLES[*].colors`
// in @chq/pricing exactly -- covered by a test golden.
